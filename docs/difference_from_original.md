...
The reason is that even if Originator Profile (OP) is implemented on a website, browsers cannot currently recognize OP-implemented sites and guide users to them as more trustworthy than sites without OP. Therefore, implementing OP offers no benefit to website operators.
...
このドキュメントでは、[Originator Profile技術研究組合（OP-CIP）](https://github.com/originator-profile)が提供する<br/>
...
その理由は、OPをWebサイトに実装しても、ブラウザがOP実装サイトを認識し、<br/>
ユーザーにOP非実装サイトよりも信頼できるサイトと案内できない現状では<br/>
OP実装は、サイト運営者に何のメリットももたらさないからです。<br/>
<br/>
しかしながら、<br/>
OPのプロパティを利用したコンテキスト広告配信は、ブラウザ依存なくWebサイト側のシステム設定で<br/>
ユーザーが閲覧するWeb記事の一部として広告を表示することができ、<br/>
ユーザーのWeb体験をより快適にすることができる可能性があります。<br/>
<br/>
OPコンテキスト広告は、記事CAと広告CAのプロパティである `genre` を<br/>
リンクさせることで実現するものですが、以下の価値が生まれます。<br/>

1. Webページへの訪問者に、そこで提供される広告も記事に関連したWeb情報の一部として提供することができる
2. その結果、広告への注目度が高まり、広告認知率、クリック率が向上します
3. 広告が記事の邪魔者にならないため、記事そのものの精読率が向上します

実際に、コンテキスト広告を実装した自身のブログ [JiJi Style](https://style.yh-inc.jp/) に掲出された12の広告の<br/>
平均クリック率は19.2％です（実装後、約1ヶ月の結果）<br/>
<br/>
紙ベースの新聞や雑誌は、広告も記事同様に、自身が提供するコンテンツの一つと位置付け、<br/>
広告主および広告表現の審査を行っていました。<br/>
しかしながら、デジタルメディアにおける広告は、メディアがプラットフォームに広告配信機能を委託し、<br/>
自らのメディアポリシーをもとにした記事上の広告配信の制御機能を手放しました。<br/>
その結果、広告が記事から阻害された存在となり、バナーブラインドネスなどの<br/>
広告そのものの情報価値が低下することとなっています。<br/>
OPコンテキスト広告の実装での良好な実験的成果は、OPコンテキスト広告がこの問題解決となることを示唆していると考えます。<br/>
<br/>
上記のOPコンテキスト広告の開発完了後、自分のWebサイトに付与したOPの発信者情報が<br/>
